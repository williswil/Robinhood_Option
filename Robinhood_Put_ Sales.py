...
def choose_most_profitable_put_option(stock_price, volatility, time_to_expiry, risk_free_rate, strike_prices):
    options = r.options.find_options_by_specific_profitability("my_stocks", expirationDate=None, strikePrice=None, optionType="put", typeProfit='chance_of_profit_short', profitFloor=0.75, profitCeiling=1.0, info=None)
    max_profit = float("-inf")
    most_profitable_strike_price = None
    for option in options:
        if option['chance_of_profit_short'] >= 0.75:
